  ...
  var Template = function (config) {
    ...
    this.calendar = function (data, monthN, yearN) {
      ...
      var rowDays = function (prev, item) {
        var cell = prev + `
          <td data-counter="${item.c}" class="calendar__day ${item.k.join(' ')}">${item.n}`;
        
        // Check if day is a period day
        if (item.k.indexOf('calendar__day--selected') !== -1 || 
            item.k.indexOf('calendar__day--selected-future') !== -1) {
          // Check if inventory exists and get items
          if (window.app && window.app.inventory) {
            var invItems = window.app.inventory.getInventory();
            invItems.forEach(function(invItem) {
              var runOut = window.app.inventory.calculateRunOut(invItem);
              cell += `<span class="inventory-icon" title="${invItem.product}: lasts ${runOut} days">
